...
export function LegalViewPersonalProjectPage() {
  ...
  const setDeedStatus = () => {
    ...
    axios.post('http://localhost/backend/setdeeddrafting.php', formData)
  .then((response) => {
    // Handle the response from the server if needed
    console.log(response.data);
    if (response.data === 'Status updated successfully') {
      alert('Deed is '+ DStatus);
      window.location.href = '/approvedprojpages';
    } else {
      alert('Failed to set');
    }
  })
      ...
      <div class="area4">
        <div>
          <div class="section">
            ...
            <select name="approvalstatus" id="approvalstatus" onChange={(e) => setDeed(e.target.value)}>
            ...
            <button id="save" class="proceed" onClick={()=>setDeedStatus()}>Save</button>
